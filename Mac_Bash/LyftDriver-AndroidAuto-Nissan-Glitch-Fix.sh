...
for tool in curl unzip; do
    if ! command -v "$tool" &>/dev/null; then
        ...
        echo ""
        echo "Both curl and unzip are pre-installed on macOS 10.x and later."
        echo "If they are missing, install them via Homebrew:"
...
# ------------------------------------------------------------
if [ ! -d "$SCRIPT_DIR/platform-tools-latest-darwin" ]; then
    echo ""
    echo "============================================================"
    ...
    curl -L --fail --progress-bar \
        -o "$SCRIPT_DIR/platform-tools-latest-darwin.zip" \
    ...
    if [ $? -ne 0 ]; then
        ...
        echo "Extract the ZIP into a folder named \"platform-tools-latest-darwin\""
        echo "so that adb ends up at:"
        echo "  platform-tools-latest-darwin/platform-tools/adb"
        echo "Then re-run the script."
        echo ""
        [ -f "$SCRIPT_DIR/platform-tools-latest-darwin.zip" ] && \
            rm -f "$SCRIPT_DIR/platform-tools-latest-darwin.zip"
        press_enter "Press Enter to EXIT..."
        exit 1
    fi
    ...
    # Extract into platform-tools-latest-darwin/ -- ZIP unfolds to platform-tools/ inside it
    # Resulting adb path: platform-tools-latest-darwin/platform-tools/adb
    mkdir -p "$SCRIPT_DIR/platform-tools-latest-darwin"
    unzip -q "$SCRIPT_DIR/platform-tools-latest-darwin.zip" \
        -d "$SCRIPT_DIR/platform-tools-latest-darwin"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo ""
        rm -rf "$SCRIPT_DIR/platform-tools-latest-darwin"
        [ -f "$SCRIPT_DIR/platform-tools-latest-darwin.zip" ] && \
            rm -f "$SCRIPT_DIR/platform-tools-latest-darwin.zip"
        press_enter "Press Enter to EXIT..."
        exit 1
...
if [ ! -d "$SCRIPT_DIR/platform-tools-latest-darwin" ]; then
    echo ""
    echo "[ERROR] The \"platform-tools-latest-darwin\" folder was NOT found."
    echo ""
    ...
    echo ""
    echo "Try deleting any leftover \"platform-tools-latest-darwin\" folder"
...
# ------------------------------------------------------------
ADB="$SCRIPT_DIR/platform-tools-latest-darwin/platform-tools/adb"
...
if [ ! -f "$ADB" ]; then
    ...
    echo ""
    echo "  TO FIX: Delete the \"platform-tools-latest-darwin\" folder and re-run"
...
export PATH="$SCRIPT_DIR/platform-tools-latest-darwin/platform-tools:$PATH"
...
echo "  $SCRIPT_DIR/platform-tools-latest-darwin/platform-tools"
echo "----------------------------------------------------"
